...
import { FaUser, FaEnvelope, FaPhone, FaIdCard, FaBuilding, FaHome, FaShieldAlt } from "react-icons/fa";
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <InfoItem icon={<FaUser />} label="Age" value={`${guardData.age} years`} />
            <InfoItem icon={<FaEnvelope />} label="Email" value={guardData.email} />
            <InfoItem icon={<FaPhone />} label="Phone" value={guardData.phone} />
            <InfoItem icon={<FaIdCard />} label="Aadhar" value={guardData.aadhar} />
          </div>

          {/* Society Information */}
          <div className="space-y-4">
            <InfoItem icon={<FaBuilding />} label="Society" value={guardData.society} />
            <InfoItem icon={<FaHome />} label="Address" value={guardData.address} />
          </div>
        </div>
          ...
          {loading ? (
            ...
            <ul className="space-y-4">
              {feedbacks.map((feedback) => (
                <li key={feedback._id} className="border p-4 rounded-lg shadow-sm bg-white">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="font-semibold">{feedback.givenBy.name}</span>
                    <span className="text-gray-500 text-sm">({feedback.givenBy.email})</span>
                  </div>
